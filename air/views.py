...
import math
from air.models import Airport, Flight, Airline, Airplane


def index(request):
    data = {}
    data['airports'] = Airport.objects.filter(active=True)
    data['airlines'] = Airline.objects.all()
    data['airplanes'] = Airplane.objects.all()
...
def convert_flight_simple(f, t):
    ...
    if f.departure == t:
        ...
        x['action'] = 'landing'
    x['planeshape'] = math.floor(((angle(f.origin, f.destination)/(math.pi*2))*360)/22)
    x['dlat'] =  (f.destination.lat - f.origin.lat)/(f.duration()/5)
    x['dlng'] =  (f.destination.lng - f.origin.lng)/(f.duration()/5)
